...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        m = max(v)
        best_idxs = [idx for idx in range(len(v)) if v[idx] == m]
    ...
    def getAction(self, state):
        ...
        action = p * random.choice(legalActions) + (not p) * self.getPolicy(state)
        return action
        util.raiseNotDefined()
    ...
    def update(self, state, action, nextState, reward):
        ...
        smpl = reward + y * self.getValue(nextState)
        v = self.values[(state, action)]
        new_v = v + a * (smpl - v)
        self.values[(state, action)] = new_v
        #util.raiseNotDefined()
